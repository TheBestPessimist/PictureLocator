...
import static pt.karambola.commons.collections.functors.FunctorUtils.coerce;
import static pt.karambola.commons.collections.functors.FunctorUtils.validate;
import static pt.karambola.commons.collections.functors.TruePredicate.truePredicate;
	...
	extends AbstractQuantifierPredicate<T>
{
    ...
    public static <T>
    Predicate<T>
    allPredicate(final Predicate<? super T>... predicates)
    {
        FunctorUtils.validate(predicates) ;

        if (predicates.length == 0)  return truePredicate();
        if (predicates.length == 1)  return coerce(predicates[0]);
    ...
    public static <T>
    Predicate<T>
    allPredicate( final Collection<? extends Predicate<? super T>> predicates )
    {
        final Predicate<? super T>[] preds = validate(predicates) ;

        if (preds.length == 0)  return truePredicate();
        if (preds.length == 1)  return coerce(preds[0]);
    ...
    evaluate(final T object)
    {
        for (final Predicate<? super T> iPredicate : iPredicates)
            if (!iPredicate.evaluate(object))  return false ;
